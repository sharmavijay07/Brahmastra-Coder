...
export default function StatusBar({ status, error, theme = 'dark' }: StatusBarProps) {
    ...
    return (
        <div className={`border-t px-3 sm:px-6 py-2 ${theme === 'light' ? 'bg-white border-gray-200' : 'bg-gray-900 border-gray-800'
            }`}>
            <div className="flex items-center gap-2">
                {getStatusIcon()}
                <span className={`text-xs sm:text-sm ${getStatusColor()} truncate`}>
                    {getStatusText()}
                </span>
            </div>
        </div>
